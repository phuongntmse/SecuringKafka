...
import org.apache.zookeeper.encryption.MatchingSchema;
...
public class ZooKeeper implements AutoCloseable {
    ...
    public List<String> getBrokersAddressList() {
        ...
        try {
            ...
            for (String id : ids) {
                String brokerInfo = new String(getData(MatchingSchema.BROKERS_PATH + "/" + id, false, null));
    ...
    public String enc_match(String topicName) {
        try {
            ...
            for (String exist_topic : topicsList) {
                ...
                if (MatchingSchema.ms_compare(exist_topic, topicName)) {
                    //get partitions
                    String partitionspath = "/brokers/topics/" + exist_topic + "/partitions";
                    List<String> partitionList = getChildren(partitionspath, false);
                    //get broker leader address
                    String partitionPath = partitionspath+"/"+partitionList.get(0)+"/state";
                    ...
                    JSONObject brokerInfo = new JSONObject(getBrokersAddressList().get(brokerID));
                    String brokerAddress = brokerInfo.getString("host") + ":" + brokerInfo.getInt("port");
                    //create response string
                    String response_str = "{\"topic_name\":\"" + exist_topic + "\",\"brokerAddress\":\"" + brokerAddress+ "\"}";
                    return response_str;
                }
            }
        } catch (Exception exception) {
